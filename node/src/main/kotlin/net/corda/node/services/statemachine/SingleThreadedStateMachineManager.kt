...
class SingleThreadedStateMachineManager(
    ...
    private inline fun <reified T : Any> tryCheckpointDeserialize(bytes: SerializedBytes<T>, flowId: StateMachineRunId): T? {
        return try {
            ...
            logger.error("Unable to deserialize checkpoint for flow $flowId. If the node has been recently updated with running flows then " +
                    "revert the update and drain the flows first. Otherwise something is very wrong and this flow will be ignored.", e)
            null
        }
    }
