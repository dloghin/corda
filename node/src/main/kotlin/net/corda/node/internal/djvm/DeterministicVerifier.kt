...
class DeterministicVerifier(
        ...
        val result = IsolatedTask(ltx.id.toString(), configuration).run {
            ...
            val sandboxTx = ltx.transform { componentGroups, serializedInputs, serializedReferences ->
                ...
                createSandboxTx.apply(arrayOf(
                    serializedInputs.toTypedArray().map { it.toSandbox(classLoader) },
                    ...
                    serializedReferences.toTypedArray().map { it.toSandbox(classLoader) }
                ))
            }
    ...
    private fun SerializedStateAndRef.toSandbox(classLoader: SandboxClassLoader): Array<Any> {
        return arrayOf(
            serializedState.deserializeFor(classLoader),
            ref.serialize().deserializeFor(classLoader)
        )
    }
